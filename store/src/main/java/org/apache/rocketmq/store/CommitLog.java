...
public class CommitLog {
    ...
    private final ThreadLocal<MessageExtBatchEncoder> batchEncoderThreadLocal; // topicQueueTable的value:  queueOffset是当前这个key中的消息数，每增加一个消息增加一(不会自减)
    protected HashMap<String/* topic-queueid */, Long/* offset */> topicQueueTable = new HashMap<String, Long>(1024); // queueOffset是当前这个key中的消息数 (key: topicId+queueId)
    ...
    public CommitLog(final DefaultMessageStore defaultMessageStore) {
        this.mappedFileQueue = new MappedFileQueue(defaultMessageStore.getMessageStoreConfig().getStorePathCommitLog(), // 存放mappedFile容器
        ...
        this.defaultMessageStore = defaultMessageStore;  // 存储工具类

        if (FlushDiskType.SYNC_FLUSH == defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) { // 同步
            this.flushCommitLogService = new GroupCommitService();
        } else { // 异步 默认异步 配置参数flushDiskType
            this.flushCommitLogService = new FlushRealTimeService();
        }
        //  CommitRealTimeService extends FlushCommitLogService
        ...
        }; // 默认useReentrantLockWhenPutMessage=false 那也就是PutMessageSpinLock锁 (其实就是cas锁)
    ...
    // 在DefaultMessageStore.start() 调用本方法   commitLog.start()
    public void start() {
        this.flushCommitLogService.start();
        // transientStorePoolEnable = true && 异步  && MASTER
        if (defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
            this.commitLogService.start();
        }
    }

    public void shutdown() { // transientStorePoolEnable = true && 异步  && MASTER
        if (defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
    ...
    public SelectMappedBufferResult getData(final long offset, final boolean returnFirstOnNotFound) {
        ...
        MappedFile mappedFile = this.mappedFileQueue.findMappedFileByOffset(offset, returnFirstOnNotFound); // 根据offset得到指定的MappedFile
        if (mappedFile != null) {
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        CompletableFuture<PutMessageStatus> flushResultFuture = submitFlushRequest(result, msg);  // todo 存放消息
    ...
    public PutMessageResult putMessage(final MessageExtBrokerInner msg) {
        ...
        try {
            long beginLockTimestamp = this.defaultMessageStore.getSystemClock().now();
            this.beginTimeInLock = beginLockTimestamp; // 开始加锁时间System.currentTimeMillis();
            ...
            result = mappedFile.appendMessage(msg, this.appendMessageCallback); // 存储消息
            switch (result.getStatus()) {
                case PUT_OK:
                    break; // 成功就结束
    ...
    public CompletableFuture<PutMessageStatus> submitFlushRequest(AppendMessageResult result, MessageExt messageExt) {
        ...
        if (FlushDiskType.SYNC_FLUSH == this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
            ...
            if (messageExt.isWaitStoreMsgOK()) { // 没配置，默认返回true，也就是走这里的逻辑
                GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes(),
                        this.defaultMessageStore.getMessageStoreConfig().getSyncFlushTimeout());
                service.putRequest(request);
                return request.future(); // 同步刷盘的任务虽然也是在异步线程中执行，但是消息存储的主流程中会同步等待刷盘结果，所以本质上还是同步操作
            } else {
                service.wakeup();
        ...
        // Asynchronous flush 异步刷盘
            ...
            if (!this.defaultMessageStore.getMessageStoreConfig().isTransientStorePoolEnable()) {
                flushCommitLogService.wakeup();  // 这里唤醒了
            } else  {
                commitLogService.wakeup(); // 这里唤醒了
            }
            return CompletableFuture.completedFuture(PutMessageStatus.PUT_OK);
        }
    }
    ...
    public long getMinOffset() {
        ...
        if (mappedFile != null) {
            if (mappedFile.isAvailable()) {
                return mappedFile.getFileFromOffset(); // 从文件开始位置
            } else {
    ...
    public SelectMappedBufferResult getMessage(final long offset, final int size) {
        int mappedFileSize = this.defaultMessageStore.getMessageStoreConfig().getMappedFileSizeCommitLog(); // commitlog 文件大小 默认1g
    ...
    // 异步刷盘 && 开启内存字节缓冲区  写消息:性能第一
    class CommitRealTimeService extends FlushCommitLogService {
        // 异步，开启TransientStorePool  CommitRealTimeService先调用mappedFile.commit, FlushRealTimeService再调用 mappedFile.flush
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                ...
                int commitDataThoroughInterval = // 默认200ms
                ...
                try {
                    ...
                    this.waitForRunning(interval); // 阻塞200ms 或提前唤醒
                } catch (Throwable e) {
    ...
    // 从上面看是异步刷盘(FlushDiskType.ASYNC_FLUSH)  写消息:性能第二
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) { // 未stop一直循环
                boolean flushCommitLogTimed = CommitLog.this.defaultMessageStore.getMessageStoreConfig().isFlushCommitLogTimed(); //  是否实时刷盘  默认实时刷盘(flushCommitLogTimed=false)
                // 刷盘间隔 默认flushIntervalCommitLog=500
                int interval = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushIntervalCommitLog();
                int flushPhysicQueueLeastPages = CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushCommitLogLeastPages(); // 最小页数 flushCommitLogLeastPages=4
                //  强制刷盘间隔
                int flushPhysicQueueThoroughInterval = // flushCommitLogThoroughInterval = 1000 * 10
                    CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushCommitLogThoroughInterval();

                boolean printFlushProgress = false;

                // Print flush progress
                long currentTimeMillis = System.currentTimeMillis(); //现在时间
                if (currentTimeMillis >= (this.lastFlushTimestamp + flushPhysicQueueThoroughInterval)) { // 判断强制刷盘时间有没有到了(上次刷盘时间+强制刷盘间隔时间 <= 现在时间 )
                    this.lastFlushTimestamp = currentTimeMillis;
                    flushPhysicQueueLeastPages = 0; // 强制刷盘时间到了，强制刷盘，不管最小页有没有达到
                    printFlushProgress = (printTimes++ % 10) == 0;
                }

                try { // 默认flushCommitLogTimed=false 默认实时刷盘
                    if (flushCommitLogTimed) { // 定时
                        Thread.sleep(interval);
                    } else { // 实时，等待500ms或者被wakeup唤醒  有数据进来，立马唤醒
                        this.waitForRunning(interval);
                    }

                    if (printFlushProgress) { // 打印
                        this.printFlushProgress();
                    }

                    long begin = System.currentTimeMillis();
                    CommitLog.this.mappedFileQueue.flush(flushPhysicQueueLeastPages); //// 交给mappedFileQueue执行flush
                    long storeTimestamp = CommitLog.this.mappedFileQueue.getStoreTimestamp();
                    if (storeTimestamp > 0) {
                        CommitLog.this.defaultMessageStore.getStoreCheckpoint().setPhysicMsgTimestamp(storeTimestamp); // 设置刷盘时间
                    }
    ...
    /** 从上面看是同步刷盘(FlushDiskType.SYNC_FLUSH) 写消息:性能第三
     * todo GroupCommit Service 同步刷盘
     */
    class GroupCommitService extends FlushCommitLogService {
        ...
        public synchronized void putRequest(final GroupCommitRequest request) {
            synchronized (this.requestsWrite) {
                this.requestsWrite.add(request);  // 放入数据
            }
            this.wakeup();  // 唤醒
        }
        ...
        private void doCommit() {
            synchronized (this.requestsRead) {
                if (!this.requestsRead.isEmpty()) { // 有数据
                    for (GroupCommitRequest req : this.requestsRead) {
                        // There may be a message in the next file, so a maximum of
                        // two times the flush 消息有可能存在2个文件
                        boolean flushOK = CommitLog.this.mappedFileQueue.getFlushedWhere() >= req.getNextOffset(); //是否刷盘成功
                        for (int i = 0; i < 2 && !flushOK; i++) {
                            CommitLog.this.mappedFileQueue.flush(0);  // 刷盘  根据flushedWhere的位置通过findMappedFileByOffset找MappedFile   flushedWhere = mappedFile.getFileFromOffset() + offset
                            flushOK = CommitLog.this.mappedFileQueue.getFlushedWhere() >= req.getNextOffset();
                        }
                        //  doCommit中每对一个request处理完成后，会调用wakeupCustomer
                        req.wakeupCustomer(flushOK ? PutMessageStatus.PUT_OK : PutMessageStatus.FLUSH_DISK_TIMEOUT); // 通知consumer
                    }

                    long storeTimestamp = CommitLog.this.mappedFileQueue.getStoreTimestamp();
                    if (storeTimestamp > 0) { // 设置存盘时间
                        CommitLog.this.defaultMessageStore.getStoreCheckpoint().setPhysicMsgTimestamp(storeTimestamp);
                    }

                    this.requestsRead.clear(); // 刷盘循环处理完，清空requestsRead列表
                } else {
        ...
        public void run() {
            ...
            while (!this.isStopped()) { // 未stop 一直循环
                try {
                    this.waitForRunning(10);  // 每隔10ms允许1次
                    this.doCommit();
                } catch (Exception e) {
                    CommitLog.log.warn(this.getServiceName() + " service has exception. ", e);
                }
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        // fileFromOffset:  本次写的开始位置    maxBlank = fileSize - currentPos = 剩余可用  单条
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            final MessageExtBrokerInner msgInner) {
            // STORETIMESTAMP + STOREHOSTADDRESS + OFFSET <br>
            // 这个byteBuffer是slice()过来的
            // PHY OFFSET
            ...
            String key = keyBuilder.toString(); // QuickOrder-2
            Long queueOffset = CommitLog.this.topicQueueTable.get(key);  // t
            if (null == queueOffset) {
                queueOffset = 0L;
            ...
            switch (tranType) { // tranType:0
                // Prepared and Rollback message is not consumed, will not enter the
                // consumer queuec
                case MessageSysFlag.TRANSACTION_PREPARED_TYPE: //
                case MessageSysFlag.TRANSACTION_ROLLBACK_TYPE:
                    queueOffset = 0L; // todo 半消息和回滚  不记录数量
                    break;
            ...
            if (propertiesLength > Short.MAX_VALUE) { // 32767
            ...
            if (msgLen > this.maxMessageSize) { // 1024 * 1024 * 4 = 4M
            ...
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {
                ...
                final long beginTimeMills = CommitLog.this.defaultMessageStore.now(); // System.currentTimeMillis();
            ...
            switch (tranType) {
                ...
                case MessageSysFlag.TRANSACTION_ROLLBACK_TYPE:
                    break;
                case MessageSysFlag.TRANSACTION_NOT_TYPE:  // 默认消息
                case MessageSysFlag.TRANSACTION_COMMIT_TYPE:
                    // The next update ConsumeQueue information
                    CommitLog.this.topicQueueTable.put(key, ++queueOffset);  // todo topicQueueTable 该key的消息数+1
                    break;
                default:
                    break;
            }
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            while (messagesByteBuff.hasRemaining()) {
                ...
                queueOffset++;  // todo queueOffset 消息数+1
                msgNum++;
