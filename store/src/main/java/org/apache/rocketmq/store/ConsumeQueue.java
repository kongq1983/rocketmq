...
public class ConsumeQueue {
    ...
    public ConsumeQueue(
        ...
        this.mappedFileSize = mappedFileSize; // 默认600w
    ...
    public void recover() {
        final List<MappedFile> mappedFiles = this.mappedFileQueue.getMappedFiles(); // 得到所有的commitlog所映射的文件
        if (!mappedFiles.isEmpty()) { // 存在文件

            int index = mappedFiles.size() - 3; //最多恢复最後三个
            if (index < 0)
                index = 0; // 小于3个，则从第1个文件开始

            int mappedFileSizeLogics = this.mappedFileSize; // 默认600w
            ...
            while (true) { // mappedFileSizeLogics 默认600w  循环1个ConsumeQueue文件的大小
                for (int i = 0; i < mappedFileSizeLogics; i += CQ_STORE_UNIT_SIZE) { // 每个consumequeue从头开始读
                    ...
                    long tagsCode = byteBuffer.getLong(); // tagsCode 消息tag 8字节

                    if (offset >= 0 && size > 0) {
                        mappedFileOffset = i + CQ_STORE_UNIT_SIZE; //发现1个entry
            ...
            // 计算总的偏移量
            processOffset += mappedFileOffset;
            this.mappedFileQueue.setFlushedWhere(processOffset); // 后续写入开始点
