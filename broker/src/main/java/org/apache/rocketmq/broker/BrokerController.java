...
public class BrokerController {
    ...
    public BrokerController(
        ...
        this.clientHousekeepingService = new ClientHousekeepingService(this); // todo 监控producer、consumer的推送 ，如果120s没推送，则删除该连接 在start()方法启动
    ...
    public boolean initialize() throws CloneNotSupportedException {
        boolean result = this.topicConfigManager.load(); // //用于管理broker中存储的所有topic的配置
        // //管理Consumer的消费进度
        result = result && this.consumerOffsetManager.load();
        result = result && this.subscriptionGroupManager.load(); //用来管理订阅组，包括订阅权限等
        result = result && this.consumerFilterManager.load(); // 用于消费者过滤配置
        ...
        if (result) {
            this.remotingServer = new NettyRemotingServer(this.nettyServerConfig, this.clientHousekeepingService); // todo 默认10911 broker port
            NettyServerConfig fastConfig = (NettyServerConfig) this.nettyServerConfig.clone();
            fastConfig.setListenPort(nettyServerConfig.getListenPort() - 2); // todo 默认10911-2=10909
            this.fastRemotingServer = new NettyRemotingServer(fastConfig, this.clientHousekeepingService);
            this.sendMessageExecutor = new BrokerFixedThreadPoolExecutor( //发送消息的线程组
            ...
            this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() { //记录broker的状态
                @Override
                public void run() {
                    try {
    ...
    public void registerProcessor() {
        ...
        // todo 注册 发送的SendMessageProcessor
        ...
        /**
         * PullMessageProcessor  todo borker 拉消息实现
         */
        ...
        this.remotingServer.registerProcessor(RequestCode.HEART_BEAT, clientProcessor, this.heartbeatExecutor); // todo 心跳clientProcessor
        ...
        this.remotingServer.registerProcessor(RequestCode.GET_CONSUMER_LIST_BY_GROUP, consumerManageProcessor, this.consumerManageExecutor); // todo GET_CONSUMER_LIST_BY_GROUP
    ...
    public synchronized void registerIncrementBrokerData(TopicConfig topicConfig, DataVersion dataVersion) {
        ...
        if (!PermName.isWriteable(this.getBrokerConfig().getBrokerPermission())
            ...
            registerTopicConfig = // 不可写 或者  不可读  会进来
                new TopicConfig(topicConfig.getTopicName(), topicConfig.getReadQueueNums(), topicConfig.getWriteQueueNums(),
                    this.brokerConfig.getBrokerPermission()); // 设置broker的权限属性
        }
    ...
    private boolean needRegister(final String clusterName,
        ...
        TopicConfigSerializeWrapper topicConfigWrapper = this.getTopicConfigManager().buildTopicConfigSerializeWrapper();
        List<Boolean> changeList = brokerOuterAPI.needRegister(clusterName, brokerAddr, brokerName, brokerId, topicConfigWrapper, timeoutMills); // 判断数据有没有变
